...
class PatchRaceBot(BotAI):
 def __init__(self,checkpoint,registry,race,smoke_steps=None):
  ...
  self.model=PatchRaceRichMTLPolicy(self.specs);self.model.load_state_dict(data['state_dict']);self.model.eval();self.c=RACE_CONFIG[race]
 def feat(self):
 ...
 async def issue(self,row):
  ...
  with torch.no_grad(): actor_scores,target_scores=self.model.pointers(self.feat(),entities[None],padding[None])
  ...
  if row['target_mode']=='point':
   ...
   with torch.no_grad(): scores=self.model.placement_scores(self.feat(),coords[None])[0]
