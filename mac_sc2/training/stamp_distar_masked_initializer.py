#!/usr/bin/env python3
"""Stamp unchanged released DI-star weights for the current masked contract."""
...
def main() -> None:
    ...
    checkpoint["current_patch_contract_hash"] = contract_hash()
    checkpoint["resumed_from"] = str(args.source.resolve())
    checkpoint["policy_action_encoding"] = "released_327_id_with_current_zerg_mask"
    args.output.parent.mkdir(parents=True, exist_ok=True)
    torch.save(checkpoint, args.output)
    print({"output": str(args.output), "contract": contract_hash(), "weights_changed": False})
